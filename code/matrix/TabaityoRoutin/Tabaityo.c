...
char *doublePow(char *n,long int r){            /*nÇÃÇíèÊÇï‘Ç∑**/
	char t[100000]={"1"};
    char x[100000]; 
           
	strcpy(x,n);
	for (; r > 0; r >>= 1, strcpy(x,doublemultiply(x,x))) {
		   if (r&1) strcpy(t,doubleINT(doublemultiply(t,x)));
    }
	mid(t,t,1,strcspn(t,"."));
	return t; 
} 

char *doubleplusminus(char *strone,char *strtwo){       //ëΩî{í∑â¡éZä÷êîÅià¯êîÇÕï∂éöóÒÇ™ÇQÇ¬Åj
	char *a,*b,*c;
	...
	char minusflag1,minusflag2,tempolalyflag;
			...
			if(periodoneb!=strlen(str1)+1){                   /*è¨êîì_ÇéÊÇËèúÇ≠*/
...
char *doublemultiply(char *strone,char *strtwo){       //ëΩî{í∑èÊéZä÷êîÅià¯êîÇÕï∂éöóÒÇ™ÇQÇ¬Åj
	char *a,*b,*c,*d;
	...
	char minusflag1,minusflag2;
			...
			if(periodone!=strlen(str1)+1){                   /*è¨êîì_ÇéÊÇËèúÇ≠*/
...
char *doubledivision(char *strone,char *strtwo){       //ëΩî{í∑èúéZä÷êîÅià¯êîÇÕï∂éöóÒÇ™ÇQÇ¬Åj
...
char *doubleINT(char *str){              /*è¨êîì_à»â∫ÇêÿÇËéÃÇƒÇÈ*/
...
char *doublemod(char *strone,char *strtwo){       //ëΩî{í∑èËó]ââéZä÷êîÅià¯êîÇÕê≥ÇÃêÆêîÇÃï∂éöóÒÇ™ÇQÇ¬Åj
int na,nb,work;
...
int doubleLeftB(char *strone,char *strtwo){      /*î‰ärââéZéqÅi>ÅjÇÃä÷êîÅAãUÇÃéûÇê^Ç∆Ç∑ÇÈÇ»ÇÁÇŒ(=<)Ç∆ÇµÇƒóòópâ¬î\*/
...
int doubleRightB(char *strone,char *strtwo){      /*î‰ärââéZéqÅi<ÅjÇÃä÷êîÅAãUÇÃéûÇê^Ç∆Ç∑ÇÈÇ»ÇÁÇŒ(=>)Ç∆ÇµÇƒóòópâ¬î\*/
int na,nb,work,periodoneb,periodtwob,periodonea,periodtwoa,nmax,x;
char *strtemporaly,*strtemporaly2,*str1,*str2;           
	        na =strlen(strone);
			nb =strlen(strtwo);
...
int doubleEqual(char* strone,char *strtwo){     /*ìôâøââéZéqÅiÅÅÅÅÅjÇÃä÷êî*/
